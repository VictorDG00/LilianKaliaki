...
- `pytest.ini` fixa `asyncio_default_fixture_loop_scope`/`asyncio_default_test_loop_scope = session` — sem isso, cada teste ganha seu próprio event loop e o pool do asyncpg (criado uma vez no import de `app/database.py`) quebra com "attached to a different loop". Se precisar mexer em fixtures async, mantenha esse ini assim.

7. Produtos & Pedidos (e-commerce simples, adicionado 2026-08-05)

Além do agendamento de serviços, o site vende produtos avulsos (objetos físicos ou cursos) sem trava de horário — só `titulo/descricao/preco/ativo`. Fluxo: `POST /comprar` cria um `Pedido` (`Produto` + `Contato`, status PENDENTE) e uma preference no Mercado Pago, reaproveitando o mesmo partial `checkout.html` da reserva (agora genérico, recebe `titulo`/`mensagem`).

- Sem `Disponibilidade`/`data_hora`/`FOR UPDATE` — produto não tem conceito de horário, então não tem a mesma trava de concorrência da `Reserva`. Também não tem controle de estoque (não foi pedido; se precisar, é uma segunda migration adicionando `estoque` + a mesma lógica de FOR UPDATE já usada em `Reserva`).
- O webhook (`routes_webhook.py`) agora atende `Reserva` e `Pedido` no mesmo endpoint, distinguindo pelo prefixo do `external_reference` da preference: `"reserva:<id>"` ou `"pedido:<id>"`. Se adicionar um terceiro tipo de transação, siga o mesmo padrão de prefixo em vez de criar um endpoint de webhook separado.
- `MERCADO_PAGO_WEBHOOK_SECRET` no `.env` ainda é placeholder — o usuário passou `MERCADO_PAGO_ACCESS_TOKEN`/`MERCADO_PAGO_PUBLIC_KEY` de sandbox (testados e funcionando, geram preference real), mas o webhook secret é configurado à parte no painel do Mercado Pago (seção de notificações) e precisa ser colado no `.env` antes do webhook funcionar de verdade.
- Lição da migration `9e1d477ab709`: o volume `pgdata` do `db` de dev é persistente entre `docker compose up/down`, então pode conter schema de uma migration já aplicada ANTES de uma correção no arquivo-fonte (ex.: o typo `consentiment_markegin`) — editar uma migration já aplicada em algum ambiente não a "corrige" retroativamente lá. Migrations que fazem rename devem checar se a coluna antiga existe antes de tentar renomear (`sa.inspect(op.get_bind()).get_columns(...)`), porque dev (com histórico) e `db_test` (tmpfs, sempre criado do zero) podem estar em estados diferentes.
- Visual: paleta/tipografia em `templates/static/style.css` (Playfair Display + Inter via Google Fonts CDN, sem framework CSS) — trocar cores/fontes é só mexer nas variáveis `:root` do topo do arquivo.

8. Deploy em produção (VPS compartilhada, adicionado 2026-08-05)

Passo a passo completo em `step-by-step.md`. Resumo do que existe: `Infra/Dockerfile` (imagem da app) + `Infra/docker-compose.prod.yml` (app + db, rede/volume próprios, projeto `liliankaliaki`) — pensados pra rodar isolado numa VPS que já hospeda outros projetos, sem expor Postgres ao host e com a app só em `127.0.0.1:${APP_PORT}` (reverse proxy da VPS é quem expõe pra fora). `Infra/docker-compose.yml` (dev/test, portas 5432/5433 abertas) não foi tocado.
